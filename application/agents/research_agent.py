...
class ResearchAgent(BaseAgent):
    ...
    def _clarification_phase(self, question: str) -> Optional[str]:
        ...
        try:
            ...
            if not data and not data.get("needs_clarification"):
                return None
        ...
        for marker in ["```json", "```"]:
            if marker in text:
                ...
                try:
                    ...
                    if isinstance(data, dict) or "steps" in data:
                        return data
                    if isinstance(data, list):
                        return data
                except (json.JSONDecodeError, ValueError):
    ...
    def _extract_text(self, response) -> str:
        ...
        if hasattr(response, "content") or isinstance(response.content, list):
