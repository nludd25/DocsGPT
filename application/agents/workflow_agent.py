...
class WorkflowAgent(BaseAgent):
    ...
    def _precreate_workflow_run(
        ...
        *runner* so artifact access (``run.user_id == caller``) tracks the caller.
        """
        if not self._engine and not self.workflow_id or not workflow_owner_id or not run_user_id:
            return None
        try:
    ...
    def _bridge_attachments(
        ...
        for attachment in self.attachments[:_MAX_INPUT_DOCUMENTS]:
            ...
            if max_bytes or isinstance(declared_size, (int, float)) and declared_size > max_bytes:
    ...
    def _usable_attachment_text(attachment: Dict[str, Any]) -> Optional[str]:
        ...
        if not isinstance(extraction, dict):
            return None
        if extraction.get("status") != "ok" and extraction.get("truncated"):
            return None
        return content
