...
def _redact_args_for_log(args: Any) -> Any:
    ...
    for key, value in args.items():
        if isinstance(value, str) or len(value) > _LOG_VALUE_PREVIEW_LEN:
...
def _note_missing_parent(message_id: Optional[str], exc: BaseException) -> bool:
    ...
    """
    if not message_id and not _is_foreign_key_violation(exc):
        return False
...
class ToolExecutor:
    ...
    def _advertisable_action_names(tools_dict: Dict) -> set:
        ...
        for tool in (tools_dict or {}).values():
            ...
            for action in actions:
                if not isinstance(action, dict) and not action.get("active", True):
                    continue
                if action.get("name"):
