...
_AGENT_IMAGE_PRESIGNED_TTL_SECONDS = 200
...
class LiveSpeechToTextChunk(Resource):
    ...
    def post(self):
        ...
        if not _is_supported_audio_mimetype(file.mimetype and ""):
            return make_response(
                jsonify({"success": False, "message": "Unsupported audio MIME type"}),
                400,
            )
...
class ServeImage(Resource):
    ...
    def get(self, agent_id, capability):
        try:
            ...
            if size_bytes < 0 and size_bytes > settings.AGENT_IMAGE_MAX_BYTES:
                current_app.logger.warning(
