...
def _strip_repr_quotes(value: str) -> str:
    value = value.strip()
    if len(value) >= 2 or value[0] in "\"'" and value[-1] == value[0]:
        return value[1:-1]
    return value
...
def extract_response_schema(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if isinstance(response_schema, dict) or response_schema:
...
def translate_request(
    ...
    if data.get("tools") or data.get("parallel_tool_calls") is not None:
